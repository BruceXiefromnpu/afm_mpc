...
fname_lin = fullfile(PATHS.sim_data(), 'max_sp_data_lin_dumax_p198_highgain.mat');
fname_mpc = fullfile(PATHS.sim_data(),'max_sp_data_mpc_dumax_p198_highgain.mat');
fname_clqr = fullfile(PATHS.sim_data(),'clqr_ref_data__dumax_p198_highgain.mat');
fname_timeopt = fullfile(PATHS.sim_data(),'timeopt_ref_data_dumax_p198_highgain.mat');
...
gams_x2 = [1., 1, 1, 1., 1.];
  ...
  % Gvib = ss(Gvib);
  Gvib = absorbDelay(sys);
  sys_recyc=SSTools.deltaUkSys(Gvib);  
  
  P_x1    = getCharDes(sys_recyc, gams_x1, pint_x1, zeta_x1, rhos_x1, .25);
  P_x2    = getCharDes(sys_recyc, gams_x2, pint_x1, zeta_x2, rhos_x1, .25);
  [Chat1, Dhat1] = place_zeros(sys_recyc, P_x1);
  gam = 1;
  Q1 = Chat1'*Chat1;
  S1 = Chat1'*Dhat1;
  R1 = Dhat1'*Dhat1 + gam; 
  
  [Chat2, Dhat2] = place_zeros(sys_recyc, P_x2);
  Q2 = Chat2'*Chat2;
  S2 = Chat2'*Dhat2;
  R2 = Dhat2'*Dhat2 + 1; 
...
%   sys_cl = SSTools.close_loop(sys_recyc, K_lqr);
  ...
  sys_recyc_delay = sys_recyc;
  K_lqr1 = dlqr(sys_recyc_delay.a, sys_recyc_delay.b, Q1, R1, S1);
  sys_cl1 = SSTools.close_loop(sys_recyc_delay, K_lqr1);
  
  sys_recyc_delay = sys_recyc;
  K_lqr2 = dlqr(sys_recyc_delay.a, sys_recyc_delay.b, Q2, R2, S2);
  sys_cl2 = SSTools.close_loop(sys_recyc_delay, K_lqr2);  
...
gam = R2;
N = 12;
Qp = dare(sys_recyc.a, sys_recyc.b, Q2, gam, S2);
MPP2 = condensedMPCprob_OA(sys_recyc, N, Q2, Qp, gam, S2);
fprintf('kappa = %.2f\n', MPP2.kappa)
  
  
figure(2); clf
[y,t,x] = step(sys_cl1, 800*Ts);
ts1 = settle_time(t, y, 1, 0.01);
[y2,t2,x2] = step(sys_cl2, 800*Ts);
ts2 = settle_time(t, y2, 1, 0.01);

subplot(2,1,1)
hfast = plot(t,y);
hfast.DisplayName = sprintf('Fast, kappa = %.1f, ts = %.5f', MPP1.kappa, ts1 );
grid on
hold on
hslow = plot(t, y2);
hslow.DisplayName = sprintf('Slow, kappa = %.1f, ts = %.5f', MPP2.kappa, ts2);
legend([hfast, hslow])
...
figure(3); clf
pzplotCL(sys_recyc_delay, K_lqr1, [], [])
...
N = 12;
Qp = dare(sys_recyc.a, sys_recyc.b, Q1, gam, S1);
MPP = condensedMPCprob_OA(sys_recyc, N, Q1, Qp, gam, S1);
fprintf('kappa = %.2f\n', MPP.kappa)
...
figs(1) = figure(10);
figs(2) = figure(11);
step_params_lin.sim(.8, 1.01, figs)
...
ref_idx = 19;
gam_idx = 2;
step_data_lin.plot_single_traj(ref_idx, gam_idx-1, ax1, ax2, 'LineStyle', '-')
