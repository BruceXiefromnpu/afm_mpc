...
The classical counterpoint to MPC's ability to explicitly handle constraints is to low-pass filter the input or to de-rate the control law (e.g., by increasing the control weight). Here, we are concerned with the latter. Despite the advances in online QP solvers, MPC is still considerably more difficult to implement requires more computation time and/or hardware resources compared to linear feedback. Thus, it is important to understand the extent to which MPC results in improved performance.

In recent work, we applied the FGM formulation of \cite{Jerez_Trans_2014} to our piezo stage and showed that, given a particular set of weighting matrices, we could increase the stabilizable range of setpoints compared to simply saturating an equivalent linear feedback \cite{braker_application_2017}.

MPC has been applied before to AFMs and, more generally, piezo actuated stages. In the Rana papers, an analog inner control loop is utilized to damp the resonances. However, the only way to guarantee control constraints in such a situation is with state constraints, which was not done ~\cite{rana_spiral_2014, rana_MPC}. Moreover, they give little detail on how the controller was tuned nor do they provide a comparison to a linear feedback law.

An alternative to MPC with respect to constraint violation is to simply relax the performance criteria, e.g., by increasing the control weight and institute a linear feedback law. Because MPC is considerably more difficult and expensive to implement, an important question is  ``how much \emph{time} is actually saved with MPC?" Explorations following this line of reasoning have not seen much attention in the applied MPC literature.



One question which naturally arises from that study is ``how much \emph{time} is actually saved with MPC?", which is the question In essence, the stabilizable set of setpoints using saturated linear feedback can be expanded by relaxing the performance criteria. 
...
Moreover, we did not give any consideration to the robustness of our scheme~\cite{braker_application_2017}.

Papers which assume state measurements: \cite{jain_design_2015}
Simulation only: \cite{Zheng_model_2013,jain_design_2015}
% Papers which say MPC but are unconstrained, so basically LQG \cite{cavanini_model_2017}
use inner damping analog control 
This is a limitation shared with other relevant applications of MPC to mechatronic systems \cite{chiying_repetitive_2017,Wills_CDC_2005, Lin_ASME_2012, rana_design_2014,jain_design_2015}. Nominally of course, MPC is a non-linear control law. However, when the solution to \eqref{eqn:optcost0} is within the interior of $\mathds{U}$, the control action is equivalent to a linear feedback law. In the situation we consider, where the constraint limits the rate of change on the control, this will always be the case as the system nears a given setpoint. Thus, within some region around any setpoint, classical ideas like gain and phase margin are directly applicable. We show in Section~\ref{sec:results} that improving these margins has a stronger influence on the experimental settle-time over a wide range of step inputs compared to a design that focuses only on increasing the aggressiveness of the control law.
