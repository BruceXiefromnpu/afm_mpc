...
% \end{subtable}
\end{table*}
Initially we assumed that selecting small values $\gamma$ would result in the best performance (i.e., fastest settling time). Determining this value of $\gamma$ requires that we define the maximum range of setpoints we would like to visit. For our stage, this is approximately a total range of $\pm35 \mu$m, which means the maximum possible step size is $70~\mu$m. Given this maximum step size, we need to determine the smallest $\gamma$ that will still yield a stable trajectory (for the nominal plant) subject to our slew rate constraint. To choose $\gamma$, we ran a series of simulations for a 70~$\mu$m step input for $\gamma$ between $10^-5$ and 10. For MPC and SLF, we take the $\gamma$ which yields the smallest settling time. Note that if $\gamma$ is small enough to result in instability, that value is automatically rejected by this metric. 
...
Using these values of $\gamma$, we tested the experimental performance of the SLF and MPC control schemes by by giving the stage a sequence of 24 reference commands. The first 20 references we selected randomly within the range $[-35~\mu$\text{m}, $35\mu\text{m}]$. We chose the final four references as $0~\mu$m, $35~\mu$m, $-35~\mu$m, and $0~\mu$m, to exercise the full range of the stage. The sequence of references is shown in Fig.~\ref{fig:steps}.
...
\textbf{Discussion:} While one might blame the lackluster performance of MPC on model uncertainty, a more focused question is ''why does the experimental performance of MPC degrade further from simulation than SLF?'' That is, why is the MPC control law less robust than SLF? To help answer this question, we looked at the gain and phase margin of controller. Reasoning that errors in both the drift and hysteresis models would present as uncertainty at low frequencies, we also considered the DC-gain of the integrated sensitivity function defined in \eqref{eqn:S_int}.
...
Fig.~\ref{fig:gmpm} plots the GM and PM of $L(z)$ vs $\gamma$. The DC-gain of $\mathcal{S}_I(z)$ is plotted as against $\gamma$ as the solid black curves in Fig.~\ref{fig:GainS}.  In both figures, the values of $\gamma$ chosen for MPC and SLF are indicated by a square and diamond, respectively. Note that for both the CS and CZ pole-placement scenarios, both GM and PM improve with larger values of $\gamma$. Moreover, the values of $\gamma$ associated with SLF result in a smaller DC-gain of $\mathcal{S}_I(z)$, which implies better robustness to low frequency disturbances.
\begin{figure}
