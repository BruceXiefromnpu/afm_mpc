...
In recent work, we applied the FGM formulation of \cite{Jerez_Trans_2014} to our piezo stage and showed that, given a particular set of weighting matrices, we could increase the stabilizable range of setpoints compared to simply saturating an equivalent linear feedback \cite{braker_application_2017}. Others have also applied MPC to similar systems \cite{Wills_CDC_2005, Lin_ASME_2012, rana_spiral_2014, rana_MPC}. In all these cases, little insight is given to how the cost function was tuned. In some cases, no comparison to linear control is given \cite{rana_spiral_2014, rana_MPC}. When a comparison to linear feedback is given, it appears this was only done for inputs of a fixed input size. This is problematic because, as we show in Section~\ref{sec:results}, it is possible to tune the weighting matrices such that MPC performs significantly better than linear feedback for only certain sizes of inputs. 

In prior research where MPC is compared to a linear feedback law \cite{Wills_CDC_2005, Lin_ASME_2012}, including in our own prior work \cite{braker_application_2017}, de-rating the linear feedback is never considered. Thus, the first question we set out to answer in this paper is ``how much performance is sacrificed by using a (possibly de-rated) linear feedback compared to MPC?''. We consider this question by looking at the settling-time for a broad range of step inputs. In section REF, we show that, in contrast to simulation results, the de-rated linear feedback has \emph{better} performance. 

Given this somewhat surprising result, the second question we seek to answer is why this should be the case, which we address in Section REF by considering the robustness of each control law. While many authors have considered robustness in MPC, many of these results assume direct measurements of the state vector \cite{santos_tool_2004, rodrigues_uncertainty_2003, kothare_robust_1996}, which is unrealistic. However, it is a well known, though perhaps under exploited, result that when the control trajectory generated by \eqref{eqn:optcost0} is within the interior of $\mathds{U}$, the control action is equivalent to an LQR-based linear feedback law.
...
We show that (up to a point) de-rating the design \emph{improves} those metrics. We argue that the nominal performance gains achieved when using a more aggressive MPC (due to constraint handling) are nullified by the concomitant decrease in robustness. We should note that it seems unlikely that we are the first to apply these classical metrics in the context of MPC; however we have not found any other papers which do so.

% We show in Section~\ref{sec:results} that improving these margins has a stronger influence on the experimental settle-time over a wide range of step inputs compared to a design that focuses only on increasing the aggressiveness of the control law.
...
\end{equation}
Due to the disturbance estimator, $\mathcal{S}(z)$ will always have a zero at $z=1$ so that its DC-gain is zero. It will be helpful in Section~\ref{sec:results} to define ''integrated sensitivity'' as
\begin{equation}
\mathcal{S}_I = \mathcal{S}\frac{1}{z-1} \label{eqn:S_int}.
\end{equation}
...
will move to the zeros of $\{A, B, F^T, 1\}$. This is illustrated in Fig.~\ref{fig:lqr_locus} for two different $F$ vectors (discussed further late in this subsection). Thus, we can effect pole-placement through the proper design of $F$ and by taking $\gamma$ to be small.
...
The second scheme, which we call ``choose-$\zeta$'', keeps the natural frequency of each complex pole unchanged but specifies a damping ratio $\zeta$. As we will see in the next section, this approach generally yields slower settling time than the constant-$\sigma$ method: however it is the same scheme we considered previously in \cite{braker_fast_2017, braker_application_2017}, so we include it here so that these results can be evaluated in light of our prior work.
...
\section{Simulated vs Experimental Performance}\label{sec:results}

% \begin{figure}
%   \includesvg[scale=1]{figures/disingenuous.svg}
%   \caption{When $\gamma=0.11$ for both MPC and SLF, MPC outperforms SLF significantly for a 6.85~$\mu$m step. When the SLF control law is de-rated such that $\gamma=12$, it performs comparably to MPC.}
%   \label{fig:disingenuous}
% \end{figure}

% When input constrained MPC is compared to a saturated linear feedback, this is often done using the same control weight for both MPC and the linear feedback and often for a limited range of inputs. Fig.~\ref{fig:disingenuous} illustrates why such an approach is insufficient. The solid red and blue trajectories are the result of tracking a 6.85 $\mu$m step command where $\gamma$ is the same for both designs. If we considered this data alone, we would conclude that MPC is far superior to SLF ''because the constraints were explicitly handled.'' However, if we slightly de-rate the SLF control by increasing $\gamma$, we get the dashed black trajectory which performs just as well. A similar result will occur for smaller reference commands but the same control weights. Thus, to comprehensively assess the potential benefit of MPC, we need to (a) choose the control weights differently so that each scheme performs the best it can and (b) evaluate performance across a range of inputs.
...
\begin{figure}
  ...
  \label{fig:steps}
\end{figure}
For both pole placement schemes, the choice of $\gamma$ has a strong influence on whether or not it makes sense to implement MPC. For example, if one takes $\gamma$ large enough, then the constrained optimal control and unconstrained optimal control will coincide for all physical setpoints making MPC and SLF equivalent. For a range of $\gamma$ smaller than this value (i.e., $\Delta u_k$ is saturated), the difference in trajectories produced by SLF and MPC is not substantial, meaning that MPC contributes little additional performance. At the other extreme, for $\gamma$ small enough, the SLF control will become unstable while MPC will not. 
Ideally, the control horizon would be long enough that any physically realizable trajectory would settle within the length of the control horizon. This would require $N$ to be several hundred, which is not currently feasible. The consequence of this is that for very small $\gamma$, the MPC may also become unstable.

\subsection{Study 1: small $\gamma$}\label{sec:small_gam}

\begin{table*}
  % \begin{minipage}{1\textwidth}
  \caption{ Settling times for the minimum-$\gamma$ control weight for both pole placement schemes.
    The first two columns show the reference value and overall change in setpoint. The first four colored columns are the result of simulation and the last four colored columns the experimental settling times. The last (un-colored row) of each table is the sum off settling times together for the respective experiment.
    % Refer to Table~\ref{tab:gmpms} for a summary of the abbreviations heading each column.
  }
  ...
  % \scriptsize
    \begin{tabular}{ccccccc|cccc}
      ...
      \input{manystepsdata_mingam.tex}
      \label{tab:ts_CSCZ1}
    \end{tabular}
    % \end{subtable}
    \begin{centering}
    ...
    \end{centering}
\end{table*}
Initially we assumed that selecting small values of $\gamma$ would result in the best performance (i.e., fastest settling time). Determining this value of $\gamma$ requires that we define the maximum range of setpoints we would like to visit. For our stage, this is approximately a total range of $\pm35 \mu$m, which means the maximum possible step size is $70~\mu$m. Given this maximum step size, we need to determine the smallest $\gamma$ that will still yield a stable trajectory (for the nominal plant) subject to our slew rate constraint. To determine this value, we ran a series of simulations for a 70~$\mu$m step input for $\gamma$ between $10^-5$ and 10. For MPC and SLF, we take the $\gamma$ which yields the smallest settling time. Note that if $\gamma$ is small enough to result in instability, that value is automatically rejected by this metric\footnote{One could, of course, conceive of other methods to determine the smallest ''reasonable'' $\gamma$. For example, one could choose the $\gamma$ which yields the shortest sum of settling times for many step sizes between 0 and 70 $\mu$m. While these other metrics will change the specific numbers cited in this section, they did not, in our experience, change the qualitative conclusion.}. 

% Note that it is not sufficient to run this set of simulations for only the largest step size because a given $\gamma$ may, for example, stabilize a step input of 70~$\mu$m but be unstable for $35~\mu$m. Thus, we run a series of simulations across a grid of $\gamma$'s. For each $\gamma$ in the grid, simulations are run for a reference of $r=2.5~\mu$m up to $r=75~\mu$m in increments of $2.5~\mu$m. If instability occurs, we move up to the next $\gamma$ in the grid. The smallest $\gamma$ which is stable for all setpoints in the range $2.5, 75$ is what we consider to be the minimum $\gamma$. This set of simulations is run for both the MPC case with $N=22$ as well the SLF case for both the constant-$\sigma$ and choose-$\zeta$ pole placement schemes.

% Additionally, faster settling times will typically be achieved by selecting a $\gamma$ larger than the liminal $\gamma$.
% % , particularly for SLF.
% To account for this, we ran a set of simulations for a 70~$\mu$m step size with $\gamma$ starting at the smallest stabilizing $\gamma$ and increasing to XX times that value. We choose the $\gamma$ which minimizes the settling time (shown in Fig. REF.). This yields $\gamma=3.5$ for the choose-$\zeta$ case and 7.5 for the constant-$\sigma$ case. 

Using these values of $\gamma$, we tested the experimental performance of the SLF and MPC control schemes by
giving the stage a sequence of 24 reference commands. The first 20 references we selected randomly within the range $[-35~\mu$\text{m}, $35\mu\text{m}]$. We chose the final four references as $0~\mu$m, $35~\mu$m, $-35~\mu$m, and $0~\mu$m, to exercise the full range of the stage. The sequence of references is shown in Fig.~\ref{fig:steps}.

For each step command, we computed the settling times, which we define in an absolute sense. Specifically, the settle boundary is 70~$\mu$m/512, which would correspond to settling within one pixel of a 512 by 512 pixel image for the given range. The settling times for each reference command are compiled into Table~\ref{tab:ts_CSCZ1}. Each row corresponds to a different reference command, as indicated in the first column. The second column shows the overall change in setpoint. Columns 3 through 6 show the simulation settling time and columns 7 through 10 show the experimental settling times. The colormap codes the settling times where dark blue is fastest flowing toward dark red as the slowest. 

\textbf{Discussion:}
In simulation, for both the constant-$\sigma$ and choose-$\zeta$ schemes, MPC provides a faster overall settling time. However, in aggregate this affect is not dramatic. As one might expect, the most benefit is seen in the larger step sizes, particularly for the CZ scheme. In the CS scheme, the  advantage of MPC is not universal, which can be seen, e.g., in the second step command, where MPC is 0.04 ms slower.

While we expected that the simulated settling times would be faster in general than the experimental times, we were surprised to see that for both the CS and CZ pole placement schemes, MPC produced noticeably longer total settling times than SLF. Moreover, the degradation from simulation is worse for the CZ scheme than CS.
In general, we observed that the MPC trajectories typically exhibited larger amounts of overshoot especially for the chosen-$\zeta$ scheme. This can be seen in the top panel of Fig~\ref{fig:min_gam_traj}, which zooms in on references 5 and 24 (which are circled in Fig.~\ref{fig:steps}).
\begin{figure}
  % \begin{minipage}{1\columnwidth}
    \includesvg[width=1\columnwidth]{figures/step_exps_min_gam.svg}
    \caption{Experimental trajectories for the constant-$\sigma$ and chosen-$\zeta$ pole placement schemes. The two columns zoom in on the two responses indicated by the red ellipses in Fig.~\ref{fig:steps}. (first row) Stage output trajectories. The dotted-black lines indicate the settle boundary. (third row) Control increment trajectories. (fourth row) Measured power amplifier current.
...
While one might blame the lackluster performance of MPC on model uncertainty, a more focused question is ''why does the experimental performance of MPC degrade further from simulation than SLF?'' That is, why is the MPC control law less robust than SLF? To help answer this question, we looked at the gain and phase margins of controllers. Reasoning that errors in both the drift and hysteresis models would present as uncertainty at low frequencies, we also considered the DC-gain of the integrated sensitivity function defined in \eqref{eqn:S_int}.

Recall that the control weight $\gamma$ in the SLF control scheme is larger than in MPC. Thus, it is particularly instructive to plot how these metrics depend on $\gamma$. 
Fig.~\ref{fig:gmpm} plots the GM and PM of $L(z)$ vs $\gamma$. The DC-gain of $\mathcal{S}_I(z)$ is plotted against $\gamma$ as the solid black curves in Fig.~\ref{fig:GainS}.  In both figures, the values of $\gamma$ chosen for MPC and SLF in the previous section are indicated by a square and diamond, respectively. Note that for both the CS and CZ pole-placement scenarios, both GM and PM improve with larger values of $\gamma$. Moreover, the values of $\gamma$ associated with SLF result in a smaller DC-gain of $\mathcal{S}_I(z)$, which implies better robustness to low frequency disturbances.
\begin{figure}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \includesvg[scale=.9]{figures/GainS_TS_vs_gamma_cs.svg}
    \caption{}
    \label{fig:GainS}
    % \end{minipage}
  \end{subfigure}
  \begin{subfigure}[t]{0.48\textwidth}
  \includesvg[scale=.9]{figures/sens_bode.svg}
  \caption{}
  \label{fig:sens_bode}
\end{subfigure}
\caption{Robustness and performance metrics for the constant-$\sigma$ and choose-$\zeta$ state weighting schemes. (a) Phase and Gain Margins as $\gamma$ is increased. (b) Gain of $\mathcal{S}_I(1)$ (solid black). The sum of simulated settling times for different $\gamma$ are indicated by red circles for SLF and black dots for MPC. Similarly, the mean of the total settling is indicated by the black and red dots with error bars.
  In both panels (a) and (b), the square and diamond markers indicated the values of $\gamma$ used in Section~\ref{sec:small_gam} for MPC and SLF, respectively. (c) Sensitivity function $\mathcal{S}$ magnitude the CS (black) and CZ (red) control design schemes.}
  \label{fig:robmetrics}
\end{figure}
\subsection{Study II: robust $\gamma$}
How might we choose $\gamma$ such that the best experimental performance results? Clearly, this choice will be a compromise between nominal speed and our different robustness metrics. To illustrate how nominal speed increases with $\gamma$, we ran a series of simulations using the sequence of steps shown in Fig.~\ref{fig:steps}. The total settling time for each simulation is shown in Fig.~\ref{fig:GainS}. Thus, while making $\gamma$ very large will improve both GM and PM, this will result in both increased sensitivity to low frequency disturbances as well as a slower nominal response. 

Notably, Fig.~\ref{fig:GainS} shows that $|\mathcal{S}_I(1)|$ has a minimum for $\gamma=50.6$ (for chosen-$\zeta$) and $\gamma=46$ (for constant-$\sigma$). We hypothesized that the fastest experimental settling times would be achieved by taking $\gamma$ near these values. To test this hypothesis, we ran a sequence of experiments for values of $\gamma$ between 0.001 and 400. Experiments at each $\gamma$ were run 5 times. The sample mean of the total settling time for each set of experiments is plotted as the black and red dots with error bars in Fig.~\ref{fig:GainS} (right axis). The error bars indicated the sample standard deviation.

\textbf{Discussion}
The fastest mean settling time for the CS scheme occurs with $\gamma=100$ and for CZ occurs with $\gamma=25$, which are slightly larger than the values of $\gamma$ which minimize the integrated sensitivity. This is likely because those larger values come with better gain margins. 
The settling times from one of the 5 experiments are compiled into Table~\ref{tab:ts_CSCZ2}. This table uses the same color map as Table~\ref{tab:ts_CSCZ1} and is organized in the same way. 
Fig.~\ref{fig:rob_gam_traj} zooms in on references 5 and 24.

In general, the CZ scheme is about 34\% slower than the CS scheme. However, as can be seen in Fig.~\ref{fig:rob_gam_traj}, the CZ scheme has much smaller residual oscillations. With these larger values of the control weight, $\Delta u_k$ is rarely saturated such that even in simulation, MPC provides no benefit. 
The largest power amplifier current measured in all experiments is 98.2 mA.

\subsection{Possible Objections}
We envision three main criticisms to these results and analysis. First, one might rightly point out that both MPC and SLF are in general, non-linear making the classical metrics like PM and GM insufficient. However, for any setpoint, both SLF and MPC behave linearly in a neighborhood around that setpoint. In other words, while the beginning of a setpoint tracking maneuver may saturate the SLF controller or put the MPC on its constraint boundary, the latter part of that same maneuver will be governed by linear dynamics. In that sense, the metrics GM and PM and the DC-gain of the integrated sensitivity still provide some insight, which we believe is manifested, e.g., in the correlation between overall settling time and $|\mathcal{S}_I(1)|$.

A related objection is that the relatively poor GMs and PMs could be improved through a loop transfer recovery (LTR) method. However, the classic method as proposed in \cite{doyle_robustness_1979} does not translate to discrete-time. Most efforts at obtaining LTR-like results for discrete-time assume that (1) a current estimator is used and (2) $CB\neq 0$ \cite{Maciejowski_asymptotic_1985,guaracy_discrete_2015,ishihara_loop_1986}. When considering MPC, especially at high sample rates, we need to use a prediction estimator, since computing the control action requires a significant portion of the sample period. Also, for our system model, $CB=0$, because the relative degree is 12. These two factors preclude discrete-time LTR. Furthermore, recall that, in contrast to the continuous-time case, the full-state feedback discrete-time LQR controller does not have a guaranteed infinite gain margin nor a guaranteed phase margin greater than $60^{\circ}$, but rather the guaranteed bounds for GM and PM depend on the DARE solution $P$ (which in turn depends on $\gamma$) \cite{andersson_moore}. 

Finally, one might argue that MPC could be beneficial if, rather than constraining the slew rate of the low-voltage input, we constrained the output of a power amplifier current model. Such an MPC problem could no longer be solved with the FGM, though with a much larger FPGA, it may be possible that the problem could be solved with the Alternating Direction Method of Multipliers (ADMM) \cite{Jerez_Trans_2014}. Such an alternative formulation would only change the constraints, not the cost. Thus, the robustness properties explored in this paper would remain unchanged, though it is possible they would play a smaller role. However, embedded platforms with FPGAs capable implementing the ADDM remain expensive. Thus, even if the state constrained problem could improve performance over SLF, this improvement would need to be weighed against the cost of simply upgrading the power amplifier. 
\begin{figure}
% \begin{minipage}{1\columnwidth}
...
\begin{table*}
  % \begin{minipage}{1\textwidth}
  \caption{Settling times for the robust-$\gamma$ control weight selection for both pole placement schemes.
    The first two columns show the reference value and overall change in setpoint. The first four colored columns are the result of simulation and the last four colored columns the experimental settling times. The last (un-colored row) of each table is the sum off settling times together for the respective experiment.
    % Refer to Table~\ref{tab:gmpms} for a summary of the abbreviations heading each column.
  }
%   \begin{subtable}{1\textwidth}
...
\section{Conclusions}
While it is well known that performance and robustness are somewhat antithetical, this fact seems little discussed in applications of MPC. Indeed, it is implicit to the claim that better performance will ensue through the use of MPC due to constraint handling that a more aggressive control will actually perform better in practice. In this paper, we have provided a counter example to that narrative. 
