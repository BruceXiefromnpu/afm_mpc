...
Finally, the slew rate limit used in the MPC/linear feedback controller must be discounted from \eqref{eqn:du_limit} to account for the inverse inverse drift and hysteresis compensatators.
...
To achieve zero offset tracking, disturbance estimators re-compute the steady state target at each time-step. For a an output disturbance ($B_d=0$ model with $C_d=I$, this simply means that the reference is adjusted by subtracting $\hat d_k$. In other words, at each time step, we need to compute
\begin{equation}
