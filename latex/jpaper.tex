...
In simulation, we found that this increases the settling time. In experiment, we found that the relaxed-criteria linear controller actually led to a faster settling time than a more aggressive MPC controller. 

Implicit in the justification for MPC (that it explicitly handles constraints) is that a stricter performance criteria actually results in improved performance. 
...
This of course requires one to decide a priori on a range of setpoints. The situation is similar in MPC, but then the link is between control horizon and setpoint size. 


While it is hardly a surprising result that de-rating a control law results in better performance through improved robustness, this effect seems little explored in the MPC literature. 

In some other cases where MPC has been applied to AFM, an analog inner control loop is utilized.  The only way to guarantee control constraints in such a situation is with state constraints, which was not done ~\cite{rana_spiral_2014, rana_MPC}. 

Although efforts have been made to characterize the robustness of MPC, those results typically assumes direct measurements of the state~\cite{santos_tool_2004, rodrigues_uncertainty_2003, kothare_robust_1996}, which is often unrealistic.
