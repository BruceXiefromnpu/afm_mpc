...
ax2 = axes('Position', [0.1100 0.1300 0.750 0.690], 'YAxisLocation', 'left');
...
hts_mpc_exp.DisplayName = 'total settle-time (MPC experiment)';

hts_slf_exp = errorbar(ts_data.gams, ts_data.means(:,1)*1000,...
  ts_data.stdv(:,1)*1000, 'k.' );
hts_slf_exp.DisplayName = 'total settle-time (SLF experiment)';


hold on
h_ts_sim = plot(gams_ts, ts_sum_sim_results(:,3)*1000, 'ro')
h_ts_sim.DisplayName = 'total settle-time (MPC simulation)';
...
exp_path = fullfile(PATHS.step_exp, 'many_steps_sweep_gamma10-Sep-2018_03');
ts_total_file = 'ts_totalconst-sig_09-10-2018.mat';
fpath = fullfile(exp_path, ts_total_file);
load(fpath)
...
ax2 = axes('Position', [0.1100 0.1300 0.750 0.690], 'YAxisLocation', 'left');
[h_sens2] = plot_sens(gams, Sens_gain2, gca(), f6,...
  'Sens_color', GM_colr, 'Sens_LS', GM_ls2);
h_sens2.DisplayName = 'Gain of $\mathcal{S}(z)/(z-1)|_{z=1}$ (choose-$\zeta$)';

% 'Ts_color', PM_colr, 'Ts_LS', PM_ls2,
yyaxis right
...
hold on
h_ts_sim2 = plot(gams_ts, ts_sum_sim_results(:,3)*1000, 'ro')
h_ts_sim2.DisplayName = 'total settle-time (simulation)';
ylabel('Settle-time [ms]')
...
leg2 = legend([h_sens2, h_ts_exp2, h_ts_sim2]);
