...
fname_frf = 'x-axis_sines_infoFourierCoef_9-27-2018-01.mat';
...
addpath('hysteresis')
fpath_driftID = fullfile(PATHS.sysid, 'hysteresis', 'driftID_data_09-27-2018_01_amp_1p0.mat');
...
figure(11); clf
stp = zpk([], [1], 1, Ts);
stp_frf = squeeze(freqresp(stp, omegas));
Efrf = squeeze(freqresp(Gvib*gdrift*stp, omegas)) - G_uz2stage_frf.*stp_frf;
e = y_exp - ydrift_est0;
plot(t_exp, e)

[Efft, freqs_] = power_spectrum(e, Ts);

figure(F4);
subplot(2,1,1)
yyaxis right
semilogx(freqs_, Efft)

%%
figure(12); clf
semilogx(freqs, abs(Efrf))
hold on
% semilogx(freqs, abs(G_uz2stage_frf));
yyaxis right
semilogx(freqs_, Efft*length(freqs_))
%%
figure(12)
Gfit_frf = squeeze(freqresp(Gvib*gdrift, omegas));
% yyaxis left

semilogx(freqs, abs(Gfit_frf), '--k')
...
hyst_file = 'hystID_data_27-Sep-2018_01.mat';
