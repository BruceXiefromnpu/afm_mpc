...
load(fullfile(PATHS.sysid, 'hysteresis', 'driftID_data_06-05-2018_01_amp_0p4.mat'))
...
%%
nsd = 2; % drift model size.
gd = canon(gdrift, 'modal');
% gd.d 
gd.c = gd.c.*gd.b';
gd.b = gd.b*0 + 1;

theta_gd0 = [diag(gd.a); gd.c'; gd.d*0.5];
theta_hyst0 = [w];
theta_sat0 = [ws];
theta0 = [theta_hyst0; theta_sat0; theta_gd0];

lb = repmat([-Inf], 1, Nhyst+Nsat+2*nsd+1);
%lb(Nhyst+1:Nhyst+1+nsd) = -1;
lb(end-2*nsd:end-2*nsd+1) = -1;
% [lb', theta0];

err_func = @(theta) hyst_drift_paralel(theta, ux, yx, tvec, Nhyst, Nsat, nsd, absorbDelay(Gvib), r, d);
